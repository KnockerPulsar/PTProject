...
int Output::GetCellStartX(const CellPosition & cellPos) const                                                  //Done - Ayman//
{
...
int Output::GetCellStartY(const CellPosition & cellPos) const                                                  //Done - Ayman//
{
	///TODO: implement the following function as described in Output.h file

	return (cellPos.VCell() + 1) * UI.CellHeight;
	// this line should be changed with your implementation
}

//////////////////////////////////////////////////////////////////////////////////////////

void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const                                    //Done - Ayman//
{
...
void Output::CreateDesignModeToolBar() const                                                   //Done - Jimmy//
{
	UI.InterfaceMode = MODE_DESIGN;
	...
	MenuItemImages[ITM_COPY_CARD] = "images\\Copy_Card.jpg";                                   //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_CUT_CARD] = "images\\Cut_Card.jpg";                                     //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_PASTE_CARD] = "images\\Paste_Card.jpg";                                 //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_EDIT_CARD] = "images\\Edit_Card.jpg";                                   //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\Delete_Card.jpg";                        //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_SAVE_GRID] = "images\\Save_Grid.jpg";                                   //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_OPEN_GRID] = "images\\Open_Grid.jpg";                                   //Done - Jimmy - Image was converted from PNG to JPG//
...
void Output::CreatePlayModeToolBar() const                                                     //Done - Jimmy//
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Input_Dice.jpg";                           //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_NEW_GAME] = "images\\New_Game.jpg";                                     //Done - Jimmy - Image was converted from PNG to JPG//
	MenuItemImages[ITM_SWITCH_TO_DESIGN_MODE] = "images\\Menu_SwitchToGrid.jpg";
	MenuItemImages[ITM_EXIT_PM] = "images\\Menu_Exit.jpg";                                     //Done - Jimmy - Image was converted from PNG to JPG//
...
void Output::PrintPlayersInfo(string info)                                                  //Done - Jimmy//
{
	///TODO: Clear what was written on the toolbar

	pWind->SetPen(UI.ToolBarColor, 1);                                                                  
	pWind->SetBrush(UI.ToolBarColor);                                                                   
	pWind->DrawRectangle( UI.MenuItemWidth*PLAY_ITM_COUNT, 0, UI.width, UI.ToolBarHeight);  //Draws a rectangle from the end of the menue items//      
	                                                                                        //to the end of the status bar to clear it         //
	...
	pWind->GetStringSize(w,h,info);                                                         //Jimmy - Added this//
	...
	pWind->DrawString(x,y,info);                                                            //Jimmy - Added this//

}

/////////////////////////////////////////////////////////////////////////////////////////

void Output::PrintInteger(int integer,string PreviousMsg) const
{
	int w=0,h=0;
	pWind->GetStringSize(w,h,PreviousMsg);
	pWind->DrawInteger(w+10, UI.height - (int)(UI.StatusBarHeight/1.3), integer);
}

////////////////////////////////////////////////////////////////////////////////////////////

void Output::PrintCell(CellPosition CellPos) const
{
	ClearStatusBar();	// First clear the status bar from any previous writing
	...
	pWind->DrawString(10,UI.height - (int)(UI.StatusBarHeight/1.3),"vCell=");
	int w1=0,h1=0;
	pWind->GetStringSize(w1,h1,"vCell=");
	pWind->DrawInteger(w1+10,UI.height - (int)(UI.StatusBarHeight/1.3),vCell);

	int w2=0,h2=0;
	pWind->GetIntegerSize(w2,h2,vCell);
	pWind->DrawString(w1+w2+20,UI.height - (int)(UI.StatusBarHeight/1.3),"hCell=");
	int w3=0,h3=0;
	pWind->GetStringSize(w3,h3,"hCell=");
	pWind->DrawInteger(w1+w2+w3+20,UI.height - (int)(UI.StatusBarHeight/1.3),hCell);
}
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const                                                   //Done - Ayman//
{
	...
	pWind->DrawRectangle(cellStartX,cellStartY,cellStartX+UI.CellWidth,cellStartY+UI.CellHeight);                        //Done - Ayman//
	...
	pWind->GetIntegerSize(w, h, cellNum);                                                                                //Done - Ayman//
	...
	pWind->DrawInteger(x, y, cellNum);                                                                                    //Done - Ayman//
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const                              //Done - Ayman//
{

	///TODO: Validate the playerNum, if not valid return

	if (playerNum >= 0 && playerNum <= 4)                                                                                   //Done - Ayman//

	{
		...
		pWind->DrawCircle(x, y, radius, FILLED);                                                                                 //Done - Ayman//
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const                                               //Done - Ayman//
{
		...
		
	{
		...
		pWind->SetPen(UI.LadderColor,UI.LadderlineWidth);                                                                               //Done - Ayman//

		///TODO: Draw The First Vertical Line (The Left Line) in the appropriate coordinates
		pWind->DrawLine(x12, y1, x12, y2, FRAME);                                                                                       //Done - Ayman//
		...
		pWind->DrawLine(x34, y1, x34, y2, FRAME);                                                                                       //Done - Ayman//

		// ---- 3- Draw the Cross Horizontal Lines ---- 
		if (y1 > y2)
		{
			for (int i = (y1-UI.CellHeight/2);i >= (y2+UI.CellHeight/2);(i -= UI.CellHeight))                 //
			{                                                                                                 //
			...
			if (y2 > y1)                                                                                      //For Ladder Horrizontal lines 
			{                                                                                                 //
				for (int i = (y2-UI.CellHeight/2);i >= (y1+UI.CellHeight/2);(i -= UI.CellHeight))             //
				{                                                                                             //
					pWind->DrawLine(x12, i, x34, i, FRAME);                                                   //
					                                                                                          //
				}                                                                                             //
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const                                //Done - Ayman//
{
	...
	if (fromCell.HCell() == toCell.HCell() && fromCell.VCell() < toCell.VCell())
	{
		...
		pWind->SetPen(UI.SnakeColor, UI.SnakelineWidth);                                                                //Done - Ayman//
		...
		arry[1] = y1 +yChange;
		arry[2] = y1;
		arry[3] = y1 - yChange;//done but not tested
		///TODO: Draw the Polygon (diamond) representing the Snake's Head
		//       Check the snakes drawn in the project document and draw it the same way
		pWind->DrawPolygon(arrx, arry, 4,FILLED);                                                                       //Done - Ayman//
