...
int Input::GetInteger(Output *pO) const 
{
	...
	///TODO: implement the GetInteger function as described in Input.h file                                     //Done - Abdullah//
	//       using function GetString() defined above and function stoi()
	string IntegerString=GetSrting(pO);
	int Integer=stoi(IntegerString);
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{	
			...
			case ITM_EXIT: return EXIT;	
				...
				///TODO: Add cases for the other items of Design Mode                                               //Done - Abdullah//
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			///(DONE) TODO: SetHCell and SetVCell of the object cellPos appropriately
			//       using the coordinates x, y and the appropriate variables of the UI_Info Object (UI)
			for(int i=0;i<NumHorizontalCells;i++)
			{
				if((x>i)&&(x<UI.CellWidth*(i+1)))
				{
					h=i;
					break;
				}
			...
			{
				if((y<(UI.ToolBarHeight+UI.CellHeight*(i+1)))&&(y>(UI.ToolBarHeight+UI.CellHeight*i)))
				{
					v=i;
					break;
				}
